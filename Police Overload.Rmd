...
# removing columns not useful to analysis
...
# crating minutes onscene variable
dispatch_data <- dispatch_data %>%
  mutate(
...
# filtering incidents to only those after 01-01-2022 (capturing post covid/riots period)
dispatch_data <- dispatch_data %>%
...
std_errs <- summary_result$standard.errors
...
#table for odds ratios (exported to LaTeX)
...
# confusion matrix using model predictions
...
```{r}
# Cox model WITH CALL VOLUME (results insignificant)
...
# running a cox model for each incident category
...
```{r}
# Cox model WITH NIGHT FLAG ONLY
summ <- list()
km_plots <- list()
zph_results <- list()
aic_results <- list()
